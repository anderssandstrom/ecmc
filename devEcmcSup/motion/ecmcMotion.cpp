...
int setAxisAutoDisableAtTgtLtch(int axisIndex, int enable) {
  LOGINFO4("%s/%s:%d axisIndex=%d enable=%d\n",
           __FILE__,
           __FUNCTION__,
           __LINE__,
  ...
  return axes[axisIndex]->setAutoDisableAtTgtLtch(enable);
}
...
int getAxisAutoDisableAtTgtLtch(int axisIndex, int *value) {
  CHECK_AXIS_RETURN_IF_ERROR(axisIndex)
  *value = axes[axisIndex]->getAutoDisableAtTgtLtch();
  return 0;
}
