...
static int handleCfgCommand(const char *myarg_1) {
  ...
  nvals = sscanf(myarg_1, "SeqNop(%d,%d)", &iValue, &iValue2);

  if (nvals == 2) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqNop");
    return setMotionSeqNop(iValue, iValue2);
  }

  /// "Cfg.SeqWaitTime(seqIndex,stepIndex,waitMs)"
  nvals = sscanf(myarg_1, "SeqWaitTime(%d,%d,%lf)", &iValue, &iValue2, &dValue);

  if (nvals == 3) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqWaitTime");
    return setMotionSeqWaitTime(iValue, iValue2, dValue);
  }

  /// "Cfg.SeqRunSeq(seqIndex,stepIndex,childSeqIndex,timeoutMs)"
  nvals = sscanf(myarg_1, "SeqRunSeq(%d,%d,%d,%lf)", &iValue, &iValue2, &iValue3, &dValue);

  if (nvals == 4) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqRunSeq");
  ...
  if (nvals == 10) {
    ...
    return setMotionSeqArmPosTrigger(iValue,
                                     iValue2,
                                     iValue3,
                                     iValue4,
                                     cIdBuffer,
  ...
  cIdBuffer[0] = '\0';
  nvals = sscanf(myarg_1,
                 "SeqArmTimeTrigger(%d,%d,%d,%[^,],%lf,%lf,%d,%lf,%lf)",
                 &iValue,
                 &iValue2,
                 &iValue3,
                 cIdBuffer,
  ...
  if (nvals == 9) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqArmTimeTrigger");
    return setMotionSeqArmTimeTrigger(iValue,
                                      iValue2,
                                      iValue3,
                                      cIdBuffer,
                                      dValue,
  ...
  nvals = sscanf(myarg_1, "SeqWaitTriggerDone(%d,%d,%d,%lf)", &iValue, &iValue2, &iValue3, &dValue);

  if (nvals == 4) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqWaitTriggerDone");
    return setMotionSeqWaitTriggerDone(iValue, iValue2, iValue3, dValue);
  }

  /// "Cfg.SeqReset(seqIndex,stepIndex,axis,timeoutMs,wait)"
  nvals = sscanf(myarg_1, "SeqReset(%d,%d,%d,%lf,%d)", &iValue, &iValue2, &iValue3, &dValue, &iValue4);

  if (nvals == 5) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqReset");
    return setMotionSeqResetWait(iValue, iValue2, iValue3, dValue, iValue4);
  }

  /// "Cfg.SeqReset(seqIndex,stepIndex,axis,timeoutMs)"
  nvals = sscanf(myarg_1, "SeqReset(%d,%d,%d,%lf)", &iValue, &iValue2, &iValue3, &dValue);

  if (nvals == 4) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqReset");
    return setMotionSeqReset(iValue, iValue2, iValue3, dValue);
  }

  /// "Cfg.SeqPower(seqIndex,stepIndex,axis,enable,timeoutMs,wait)"
  nvals = sscanf(myarg_1,
                 "SeqPower(%d,%d,%d,%d,%lf,%d)",
                 &iValue,
                 &iValue2,
                 &iValue3,
                 &iValue4,
  ...
  if (nvals == 6) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqPower");
    return setMotionSeqPowerWait(iValue, iValue2, iValue3, iValue4, dValue, iValue5);
  }

  /// "Cfg.SeqPower(seqIndex,stepIndex,axis,enable,timeoutMs)"
  nvals = sscanf(myarg_1, "SeqPower(%d,%d,%d,%d,%lf)", &iValue, &iValue2, &iValue3, &iValue4, &dValue);

  if (nvals == 5) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqPower");
    return setMotionSeqPower(iValue, iValue2, iValue3, iValue4, dValue);
  }

  /// "Cfg.SeqHome(seqIndex,stepIndex,axis,homeSeq,homePos,velTowardsCam,velOffCam,acc,dec,timeoutMs,wait)"
  nvals = sscanf(myarg_1,
                 "SeqHome(%d,%d,%d,%d,%lf,%lf,%lf,%lf,%lf,%lf,%d)",
                 &iValue,
                 &iValue2,
                 &iValue3,
                 &iValue4,
  ...
  if (nvals == 11) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqHome");
    return setMotionSeqHomeWait(iValue,
                                iValue2,
                                iValue3,
                                iValue4,
                                dValue,
  ...
  nvals = sscanf(myarg_1,
                 "SeqHome(%d,%d,%d,%d,%lf,%lf,%lf,%lf,%lf,%lf)",
                 &iValue,
                 &iValue2,
                 &iValue3,
                 &iValue4,
  ...
  if (nvals == 10) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqHome");
    return setMotionSeqHome(iValue,
                            iValue2,
                            iValue3,
                            iValue4,
                            dValue,
  ...
  nvals = sscanf(myarg_1,
                 "SeqMoveAbs(%d,%d,%d,%lf,%lf,%lf,%lf,%lf,%d)",
                 &iValue,
                 &iValue2,
                 &iValue3,
                 &dValue,
  ...
  if (nvals == 9) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqMoveAbs");
    return setMotionSeqMoveAbsWait(iValue,
                                   iValue2,
                                   iValue3,
                                   dValue,
                                   dValue2,
  ...
  nvals = sscanf(myarg_1,
                 "SeqMoveAbs(%d,%d,%d,%lf,%lf,%lf,%lf,%lf)",
                 &iValue,
                 &iValue2,
                 &iValue3,
                 &dValue,
  ...
  if (nvals == 8) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqMoveAbs");
    return setMotionSeqMoveAbs(iValue, iValue2, iValue3, dValue, dValue2, dValue3, dValue4, dValue5);
  }

  /// "Cfg.SeqMoveRel(seqIndex,stepIndex,axis,dist,vel,acc,dec,timeoutMs,wait)"
  nvals = sscanf(myarg_1,
                 "SeqMoveRel(%d,%d,%d,%lf,%lf,%lf,%lf,%lf,%d)",
                 &iValue,
                 &iValue2,
                 &iValue3,
                 &dValue,
  ...
  if (nvals == 9) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqMoveRel");
    return setMotionSeqMoveRelWait(iValue,
                                   iValue2,
                                   iValue3,
                                   dValue,
                                   dValue2,
  ...
  nvals = sscanf(myarg_1,
                 "SeqMoveRel(%d,%d,%d,%lf,%lf,%lf,%lf,%lf)",
                 &iValue,
                 &iValue2,
                 &iValue3,
                 &dValue,
  ...
  nvals = sscanf(myarg_1, "SeqHalt(%d,%d,%d,%lf,%d)", &iValue, &iValue2, &iValue3, &dValue, &iValue4);

  if (nvals == 5) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqHalt");
    return setMotionSeqHaltWait(iValue, iValue2, iValue3, dValue, iValue4);
  }
  ...
  if (nvals == 4) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqWaitInPos");
  ...
  if (nvals == 5) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("SeqSetItem");
    return setMotionSeqSetItem(iValue, iValue2, cIdBuffer, dValue, dValue2);
  }
  ...
  /* Cfg.EcAddEntryCyclicWrite(toEntryPath,fromEntryPath,force) */
  cIdBuffer[0]  = '\0';
  cIdBuffer2[0] = '\0';
  nvals = sscanf(myarg_1,
                 "EcAddEntryCyclicWrite(%[^,],%[^,],%d)",
                 cIdBuffer,
                 cIdBuffer2,
                 &iValue);

  if (nvals == 3) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("EcAddEntryCyclicWrite");
    return ecAddEntryCyclicWrite(cIdBuffer, cIdBuffer2, iValue);
  }
  ...
  cIdBuffer2[0] = '\0';
  nvals = sscanf(myarg_1,
                 "EcAddEntryCyclicWrite(%[^,],%[^)])",
                 cIdBuffer,
                 cIdBuffer2);

  if (nvals == 2) {
    RETURN_ERROR_IF_RUNTIME_CFG_CMD("EcAddEntryCyclicWrite");
    return ecAddEntryCyclicWrite(cIdBuffer, cIdBuffer2, 0);
  }
